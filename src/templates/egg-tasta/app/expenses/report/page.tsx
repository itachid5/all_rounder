import { ChevronRight, BarChart3, Calendar, PieChart, Layers } from "lucide-react";
import Link from "next/link";

export default function ExpenseReportPage() {
  const reports = [
    { title: "Daily Expense", icon: Calendar, description: "View all expenses made today." },
    { title: "Monthly Expense", icon: BarChart3, description: "Analyze expense trends over the month." },
    { title: "Category-wise Expense", icon: Layers, description: "Breakdown of expenses by category." },
    { title: "Expense Summary", icon: PieChart, description: "High-level summary of total business expenses." },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Analyze your outgoing business expenses and generate reports.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          ...
          <div className="flex flex-col">
            <label className="text-xs font-medium text-slate-500 mb-1">Category Filter</label>
            <select className="px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reports.map((report, idx) => (
          <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm opacity-60">
            <div className="h-12 w-12 rounded-lg bg-orange-50 dark:bg-orange-900/20 text-orange-600 dark:text-orange-400 flex items-center justify-center mb-4">
