import { ChevronRight, DollarSign, Building, Smartphone, ArrowRightLeft, PieChart } from "lucide-react";
import Link from "next/link";
import { EmptyState } from "@/templates/egg-shop/components";

export default function AccountReportPage() {
  const reports = [
    { title: "Balance Summary", icon: PieChart, description: "Total balances across all accounts." },
    { title: "Cash Report", icon: DollarSign, description: "Detailed summary of cash movements." },
    { title: "Bank Report", icon: Building, description: "Summary of all bank transactions." },
    { title: "Mobile Banking Report", icon: Smartphone, description: "Summary of mobile wallet transactions." },
    { title: "Transfer Report", icon: ArrowRightLeft, description: "Log of all internal fund transfers." },
  ];
    ...
    <div className="space-y-6">
      ...
      <div>
        <nav className="flex items-center text-sm text-slate-500 dark:text-slate-400 mb-2">
          ...
          <ChevronRight className="h-4 w-4 mx-1" />
          <span className="text-slate-900 dark:text-slate-100 font-medium">Account Report</span>
        </nav>
        
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white">Account Report</h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">Analyze financial movements across all accounts.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg p-4 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-4 w-full md:w-auto">
          ...
          <div className="flex flex-col">
            <label className="text-xs font-medium text-slate-500 mb-1">Account Filter</label>
            <select className="px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              <option value="">All Accounts</option>
              <option value="cash">Cash Only</option>
              <option value="bank">Banks Only</option>
              <option value="mobile">Mobile Banking Only</option>
            </select>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reports.map((report, idx) => (
          <div key={idx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm opacity-70 hover:opacity-100 transition-opacity cursor-pointer">
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center text-sm text-slate-400">
              <span>View Report Details &rarr;</span>
            </div>
          </div>
        ))}
      </div>
