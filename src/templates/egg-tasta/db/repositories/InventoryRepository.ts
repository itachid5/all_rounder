...
import { stockAdjustments, inventoryMovements, products } from "@/templates/egg-tasta/db/schema";

import { eq, and, desc, sql, like, or } from "drizzle-orm";
import { randomUUID } from "crypto";

export class InventoryRepository {
  static async generateAdjustmentNo(tenantId: string, tx: any = db): Promise<string> {
  ...
  static async createStockAdjustment(tenantId: string, data: any, userId?: string) {
    return await db.transaction(async (tx) => {
          const adjustmentNo = await this.generateAdjustmentNo(tenantId, tx);
          const adjustmentId = randomUUID();
          const date = data.date ? new Date(data.date) : new Date();

          const product = await tx.select().from(products).where(and(eq(products.tenantId, tenantId), eq(products.id, data.productId))).get();
  ...
  static async listStockAdjustments(tenantId: string, options: { search?: string, page?: number, limit?: number } = {}) {
  ...
  static async getStockValuation(tenantId: string) {
    ...
    const allProducts = await db.select().from(products).where(and(eq(products.tenantId, tenantId), eq(products.isDeleted, false))).all();
    const allInMovements = await db.select().from(inventoryMovements).where(and(eq(inventoryMovements.tenantId, tenantId), eq(inventoryMovements.type, 'IN'))).all();
