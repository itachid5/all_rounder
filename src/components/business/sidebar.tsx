...
import { Building2, X, ChevronDown, LayoutDashboard, Layers, Users, Shield, Activity, Settings, MoveRight, DollarSign, Bell, HandCoins, WalletCards } from "lucide-react";
    ...
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-50 border-r border-slate-200 dark:border-slate-800 flex flex-col transition-transform duration-200 ease-in-out ${open ? "translate-x-0" : "-translate-x-full"} lg:translate-x-0 lg:static`}>
        ...
        <div className="p-3 border-t border-slate-200 dark:border-slate-800 space-y-1">
          <Link href="/app/profile" className={`flex items-center gap-3 px-3 py-2 rounded-md transition-colors text-left ${pathname === '/app/profile' ? 'bg-blue-50 text-blue-700 font-medium dark:bg-blue-900/30 dark:text-blue-400' : 'text-slate-600 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white'}`}>
