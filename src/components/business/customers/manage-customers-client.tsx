...
export function ManageCustomersClient({ initialData, initialTotal }: { initialData: any[], initialTotal: number }) {
  ...
  const handleArchive = async (code: string) => {
    ...
    await updateCustomerStatusAction([code], 'ARCHIVED');
    const res = await listCustomersAction({ search, status: statusFilter, sortBy, sortDir, page, limit });
    if (res.success) {
      setData(res.data);
      setTotal(res.total);
                  ...
                  <Td className="text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      {item.status === 'ARCHIVED' ? (
                        ...
                        <button onClick={() => handleArchive(item.customerCode)} className="p-1.5 text-slate-400 hover:text-amber-600 hover:bg-amber-50 rounded dark:hover:text-amber-400 dark:hover:bg-amber-900/30 transition-colors" title="Archive">
